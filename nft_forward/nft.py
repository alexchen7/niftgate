...
def render_nft(settings: Settings, state: State) -> str:
    ...
    if guarded:
        ...
        for rule in guarded:
            ...
            if sources:
                ...
                lines += [
                    f'        ip saddr != @{set_name} tcp dport {rule.lport} limit rate 12/minute burst 20 packets log prefix "{prefix}"',
                    f"        ip saddr != @{set_name} tcp dport {rule.lport} drop",
                    f'        ip saddr != @{set_name} udp dport {rule.lport} limit rate 12/minute burst 20 packets log prefix "{prefix}"',
                    f"        ip saddr != @{set_name} udp dport {rule.lport} drop",
                ]
            else:
                lines += [
                    f'        tcp dport {rule.lport} limit rate 12/minute burst 20 packets log prefix "{prefix}"',
                    f"        tcp dport {rule.lport} drop",
                    f'        udp dport {rule.lport} limit rate 12/minute burst 20 packets log prefix "{prefix}"',
                    f"        udp dport {rule.lport} drop",
                ]
        lines += ["    }", ""]
    lines += [
        "    chain prerouting {",
    ...
    for rule in rules:
        ...
        if rule.note:
            lines.append(f"        # 备注: {' '.join(rule.note.splitlines()).strip()}")
        if rule.open_access:
            src_match = ""
    ...
    for rule in rules:
        lines += [
            "",
            f"        # 回源: 发往 {rule.dest_ip}:{rule.dest_port} 的已 DNAT 流量, 使用出口接口地址",
            f"        ip daddr {rule.dest_ip} tcp dport {rule.dest_port} ct status dnat masquerade",
            f"        ip daddr {rule.dest_ip} udp dport {rule.dest_port} ct status dnat masquerade",
        ]
    lines += ["    }", "}"]
    return "\n".join(lines) + "\n"
